    ...
    <div class="fd-nav-content">
      <div class="Utab">
        ...
        <div class="Userchinput">
          <el-input v-model="searchValue" placeholder="输入标题"><i slot="prefix"
      ...
      <div class="UtableDiv">
        <el-table :data="tableData" style="width: 100%;" border>
            ...
            <el-table-column label="流程状态" align="center">
                ...
                <span v-show="scope.row.status=='0'" style="color:#5473e8;">审核中</span>
                <span v-show="scope.row.status=='1'" style="color:#23bcca;">审核通过</span>
                <span v-show="scope.row.status=='2'" style="color:#f56c6c;">审核失败</span>
                </template>
            </el-table-column>
            <el-table-column label="发起时间" prop="starttime" align="center" :formatter="dateFormatter"></el-table-column>
            <el-table-column label="结束时间" prop="endtime" align="center" :formatter="dateFormatter"></el-table-column>
...
<script>
  export default {
    ...
    methods: {
      queryTaskLists(){
        // 查询我发起的数据
        this.$axios.get('/api/qdc/querySponsorTasks', {
          params: {
            userid: "sa"
          }
        }).then(res => {
      ...
      doFilter() {
        ...
        this.resData.filter((item) => {
          // debugger
          if ('formname' in item || 'starttime' in item || 'endtime' in item || "status" in item) {
            if(taskstatusVal == item.status){
              if (this.searchValue == null) {
                if (new Date(item.starttime).getTime() >= starTime && new Date(item.endtime).getTime() <= endTime) {
                  this.filterTableData.push(item);
                }
              } else if (this.searchValue != null) {
                if (Number.isNaN(starTime)) { //true
                  if (item.formname.indexOf(this.searchValue.toUpperCase()) > -1) {
                    this.filterTableData.push(item);
                  }
                } else {
                  if (item.formname.indexOf(this.searchValue.toUpperCase()) > -1 &&
                    (new Date(item.starttime).getTime() >= starTime && new Date(item.endtime).getTime() <= endTime)) {
                    this.filterTableData.push(item);
                  }
                }
              }
